...
## Data
- `src/data/unitPotential.json` — every unit's fully-researched ceiling (stats + max stack size) computed from datamined research modifiers. Filter semantics: `filterType 2` = specific troop(s) by languageKey, `filterType 3` = race/status-wide; the sibling field in each case is stale filler. Status ints map 0=Human 1=Faey 2=Undead 3=Musician 4=Rana 5=Beast 6=Harima 7=Garrison 8=Ranged 9=Vildra 10=Roots. Powers the compare page's "Max potential" mode.
- Yulan lines that upgrade per house use `upgrades: [{house, ...}]` instead of `upgraded` — three variants (Xuan=arcana, Sheng=creation, Li=order) from the datamined `arcanaUpgraded`/`creationUpgraded`/`orderUpgraded` tiers. The compare table emits one row each; faction cards render a 4-column line.
- `src/data/units.json` — 65 unit lines / 130 table rows across all 7 factions (Arleon, Rana, Barya, Barony of Loth, Vanir, Roots, Yulan) scraped from the fandom wiki faction pages via the MediaWiki API on 2026-08-08. Yulan's Grenadier and Seeker lines have no portraits (none exist on the wiki). Some units genuinely produce doubled essence (e.g. Musketeer 2× Order) — not a scrape artifact.
